...
$outputName = "AndreaNeoNovelDownloader"
$buildPath = "build"
$sourceFile = "main.py"
...
if ($output.Length -gt 100) {
    python3 -m nuitka --follow-imports --standalone --show-memory  --show-progress --include-package=requests --output-dir=$buildPath $sourceFile
} else {
    nuitka --follow-imports --standalone --show-memory  --show-progress --include-package=requests --output-dir=$buildPath $sourceFile
}
